...
class Results(Table):
    dep_id = Col('dep_id', show=True)
    logger_id = Col('logger_id')
    installation_date = Col('installation_date')
...
#    volt_status = Col('Volt Status')
    logger_name = LinkCol('logger_name', 'show', url_kwargs=dict(name ='logger_name'), attr='logger_name')
